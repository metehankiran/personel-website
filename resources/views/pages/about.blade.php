...
@section('title', 'Hakkımda — ' . $general->author_name)
        ...
        <div class="split-2 split-content">
            <div>
                ...
                <div style="margin-top:64px;">
                    ...
                    <div class="timeline-row"><span class="timeline-year">2026</span><span class="timeline-role">Şu an</span><span class="timeline-desc">Karavela v2 üzerinde çalışıyorum. {{ $general->availability_status ?? 'Yeni proje alıyor' }}.</span></div>
                ...
                <div class="sidebar-card">
                    ...
                    <div style="display:flex;flex-direction:column;gap:12px;font-size:13px;">
                        @if($general->author_location)
                            <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border);padding-bottom:10px;"><span class="sub">Lokasyon</span><span style="font-weight:500;">{{ $general->author_location }}</span></div>
                        @endif
                        ...
                        @if($general->homepage_stats)
                            @foreach(collect($general->homepage_stats)->whereIn('label', ['Tecrübe']) as $stat)
                                <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border);padding-bottom:10px;"><span class="sub">{{ $stat['label'] }}</span><span style="font-weight:500;">{{ $stat['value'] }}</span></div>
                            @endforeach
                        @endif
                    </div>
