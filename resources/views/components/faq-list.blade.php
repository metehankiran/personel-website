...
<div {{ $attributes->class(['border-t border-neutral-200 dark:border-neutral-800']) }}>
    @foreach($faqs as $faq)
        <details class="group border-b border-neutral-200 dark:border-neutral-800" @if($loop->first) open @endif>
            <summary class="flex items-center justify-between gap-6 py-5 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                <h3 data-speakable="question" class="m-0 text-[17px] font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">{{ $faq->question }}</h3>
            ...
            <p data-speakable="answer" class="m-0 pb-6 pr-10 text-[15px] leading-[1.7] text-neutral-600 dark:text-neutral-400">{!! nl2br(e($faq->answer)) !!}</p>
        </details>
    @endforeach
</div>
