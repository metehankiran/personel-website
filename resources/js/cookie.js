...
if (!stored || !stored.ts) {
    ...
    createIcons({ icons, nodes: [banner] });

    function decide(accepted) {
        try {
            localStorage.setItem(KEY, JSON.stringify({ analytics: accepted, functional: true, ts: new Date().toISOString() }));
        } catch {}
        banner.style.animation = 'cookieOut .2s ease-in forwards';
        setTimeout(() => banner.remove(), 250);
        if (accepted && typeof window.mkEnableAnalytics === 'function') window.mkEnableAnalytics();
    }
